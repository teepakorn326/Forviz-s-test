...
import DateOptional from "../components/DateOptional";
import ScheduleOptional from "../components/ScheduleOptional";
...
function Optional() {
  ...
  const roomName = pathname.split("/");
  const roomIdName = roomName[2];

  const getRoomById = bookingData.filter((item) => item.roomId === roomIdName);
  console.log("getRoomById", getRoomById);
  return (
    <div className="flex flex-row justify-between">
      <DateOptional getRoomById={getRoomById} roomIdName={roomIdName} />
      <ScheduleOptional />
    </div>
  );
}
